...
VALUES
('017.503.005-61','MARIZA FAUSTO DA SILVA','8176152703','81591527003','marizafaustodasilva@gmail.com'),
...
-- 2
SELECT e.nome "Empregado", COUNT(v.idVenda) "NumeroDeVendas", FORMAT(SUM(v.valor), 2, 'de_DE') AS TotalVendas
FROM Venda v
JOIN Empregado e ON v.Empregado_cpf = e.cpf
GROUP BY e.nome
ORDER BY TotalVendas DESC;
-- 3
SELECT d.nome AS "Departamento", COUNT(e.cpf) AS "Quantidade de Empregados", AVG(e.salario) AS "Média Salarial", AVG(e.comissao) AS "Média da Comissão"
FROM petshop.Departamento d
LEFT JOIN petshop.Empregado e ON d.idDepartamento = e.Departamento_idDepartamento
GROUP BY d.nome
ORDER BY d.nome;


-- 4
SELECT e.nome AS 'Nome Empregado', e.cpf AS 'CPF Empregado', e.sexo AS 'Sexo', e.salario AS 'Salário', COUNT(v.idVenda) AS 'Quantidade Vendas', SUM(v.valor) AS 'Total Valor Vendido', SUM(v.comissao) AS 'Total Comissão das Vendas'
FROM petshop.Empregado e
LEFT JOIN petshop.Venda v ON e.cpf = v.Empregado_cpf
GROUP BY e.cpf
ORDER BY COUNT(v.idVenda) DESC;

-- 6

SELECT p.nome AS "Nome do Pet", s.data AS "Data do Serviço", 
       se.nome AS "Nome do Serviço", s.quantidade AS "Quantidade", 
       s.valor AS "Valor", e.nome AS "Empregado que realizou o Serviço"
FROM petshop.PET p
JOIN petshop.itensServico s ON p.idPET = s.PET_idPET
JOIN petshop.Servico se ON s.Servico_idServico = se.idServico
JOIN petshop.Empregado e ON s.Empregado_cpf = e.cpf
ORDER BY s.data DESC;

-- 10
SELECT
    v.data "Data Venda", COUNT(v.idVenda) "Quantidade de Vendas", SUM(v.valor) "Valor Total Venda"
FROM petshop.Venda v
GROUP BY v.data
ORDER BY v.data DESC;

-- 1
SELECT e.nome "Nome Empregado", e.cpf "CPF Empregado", e.dataAdm "Data Admissão", e.salario "Salário", d.nome "Departamento", t.numero "Número de Telefone"
JOIN petshop.Departamento d ON e.Departamento_idDepartamento = d.idDepartamento
JOIN petshop.Telefone t ON e.cpf = t.Empregado_cpf
WHERE e.dataAdm BETWEEN '2019-01-01' AND '2022-03-31'
ORDER BY e.dataAdm DESC;

-- 5 

SELECT e.nome "Nome Empregado", e.cpf "CPF Empregado", COUNT(v.idVenda) "Quantidade Vendas com Serviço", SUM(v.valor) "Total Valor Vendido com Serviço"
FROM petshop.Empregado e
JOIN petshop.Venda v ON e.cpf = v.Empregado_cpf
WHERE v.tipoVenda = 'Serviço'  -- Supondo que exista um campo para tipo de venda
GROUP BY e.cpf
ORDER BY COUNT(v.idVenda) DESC;

-- 7
SELECT v.data "Data da Venda", v.valor "Valor", v.desconto "Desconto", v.valorFinal "Valor Final", e.nome "Empregado que realizou a venda"
FROM petshop.Venda v
JOIN petshop.Empregado e ON v.Empregado_cpf = e.cpf
ORDER BY v.data DESC;

-- 8
SELECT s.nome "Nome do Serviço", COUNT(v.idVenda) "Quantidade Vendas", SUM(v.valor) "Total Valor Vendido"
FROM petshop.Servico s
JOIN petshop.Venda v ON s.idServico = v.Servico_idServico
GROUP BY s.idServico
ORDER BY COUNT(v.idVenda) DESC
LIMIT 10;

-- 9
SELECT p.tipo "Tipo Forma Pagamento", COUNT(v.idVenda) "Quantidade Vendas", SUM(v.valor) "Total Valor Vendido"
FROM petshop.Pagamento p
JOIN petshop.Venda v ON p.idPagamento = v.Pagamento_idPagamento
GROUP BY p.idPagamento
ORDER BY COUNT(v.idVenda) DESC;

-- 11
SELECT p.nome "Nome Produto", p.valor "Valor Produto", p.categoria "Categoria do Produto", f.nome "Nome Fornecedor", f.email "Email Fornecedor", f.telefone "Telefone Fornecedor"
FROM petshop.Produto p
JOIN petshop.Fornecedor f ON p.Fornecedor_idFornecedor = f.idFornecedor
ORDER BY p.nome;

-- 12
SELECT p.nome "Nome Produto", COUNT(v.idVenda) "Quantidade (Total) Vendas", SUM(v.valor) "Valor Total Recebido pela Venda do Produto"
FROM petshop.Produto p
JOIN petshop.Venda v ON p.idProduto = v.Produto_idProduto
